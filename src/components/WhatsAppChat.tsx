...
export default function WhatsAppChat() {
  ...
  const whatsappBase = config.contact.whatsappUrl || "https://wa.me/2347036809459";
  ...
  const getWhatsAppUrl = (text: string) => {
    ...
    if (whatsappBase.includes("?")) {
      return `${whatsappBase}&text=${encoded}`;
    }
    return `${whatsappBase}?text=${encoded}`;
  };
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      ...
      {open && (
        <div
          className="flex w-[90vw] max-w-[380px] flex-col overflow-hidden rounded-3xl border border-gold/30 bg-cream shadow-2xl"
